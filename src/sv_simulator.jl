...
Includes all the outputs and classical bit values generated by mid circuit measurements throughout the simulation.
"""
...
"""
    get_measurement_probabilities(state::AbstractVector{<:Complex}, qubit::Int)

Calculate measurement probabilities for a qubit in a state vector.
...
function get_measurement_probabilities(state::AbstractVector{<:Complex}, qubit::Int)
    ...
    for i in 0:(length(state)-1)
        ...
        prob = abs2(state[i+1])  # Probability of this basis state
...
"""
    _apply_reset(state::AbstractVector{<:Complex}, target::Int)

Reset a qubit to |0⟩ state in a state vector.
This projects the state to the |0⟩ state of the target qubit and then normalizes.
Returns the new state.
"""
function _apply_reset(state::AbstractVector{<:Complex}, target::Int)
    ...
    if outcome == 1
        ...
        # Apply X gate to flip the target qubit
...
"""
    _apply_reset!(simulator::StateVectorSimulator, target::Int)

Reset a qubit to |0⟩ state in a state vector simulator.
This projects the state to the |0⟩ state of the target qubit and then normalizes.
"""
function _apply_reset(simulator::StateVectorSimulator, target::Int)
    return _apply_reset(simulator.state_vector, target)
end

"""
    apply_projection(state::AbstractVector{<:Complex}, qubit::Int, outcome::Int)

Apply measurement projection to a state vector.
This collapses the state to the subspace corresponding to the measurement outcome.
Returns the collapsed state. If all qubits are measured, the returned state is an empty list.
Throws an error if the qubit outcome isn't possible.
"""
function apply_projection(state::AbstractVector{<:Complex}, qubit::Int, outcome::Int)
    n_qubits = Int(log2(length(state)))
    
    # If there are no qubit present in the state (they have all already been measured)
    if n_qubits == 0
    ...
    # Create a new statevector with half the size
    new_size = length(state) ÷ 2
    new_state = zeros(ComplexF64, new_size)
    
    # Copy only the amplitudes corresponding to the measurement outcome
    counter = 0
    outcome_exists = false
    for i in 0:(new_size*2-1)
        # Calculate the original index in the full statevector
        # We need to insert the measurement outcome bit at the right position
        qubit_val = (i >> endian_qubit) & 1
        if qubit_val == outcome
            new_state[counter+1] = state[i+1]
            counter += 1
            outcome_exists = true
        end
    end
...
"""
    apply_projection!(simulator::StateVectorSimulator, qubit::Int, outcome::Int)
...
function apply_projection(simulator::StateVectorSimulator, qubit::Int, outcome::Int)
    return apply_projection(simulator.state_vector, qubit, outcome)
end

"""
    expand_state(state::AbstractVector{<:Complex}, qubit::Int, outcome::Int)
...
"""
function expand_state(state::AbstractVector{<:Complex}, qubit::Int, outcome::Int)
    n_qubits = Int(log2(length(state)))
    
    # Create a new statevector with twice the size
    new_size = length(state) * 2
    new_state = zeros(ComplexF64, new_size)
    
    # Determine where to insert the qubit in the new statevector
    # By default, insert at the end (MSB)
...
"""
    add_qubits(svs::StateVectorSimulator, num_qubits::Int) -> StateVectorSimulator

Add `num_qubits` qubits to the state vector simulator, initializing them in the |0⟩ state.
Returns a new simulator with the expanded state.

# Arguments
- `svs`: The state vector simulator to expand
- `num_qubits`: The number of qubits to add

# Returns
- A new `StateVectorSimulator` with the expanded state
"""
function add_qubits(state::AbstractVector{<:Complex}, num_qubits::Int)
    # Validate input
    ...
    # Handle empty state vector case - initialize with |0⟩ state
    if isempty(state)
        # Create a state vector with 2^num_qubits elements
        T = eltype(state)
        new_state = zeros(T, 2^num_qubits)
        new_state[1] = one(T)  # Set |0⟩ state
        return new_state
    end
    
    # Calculate new dimensions
    old_size = length(state)
    new_size = old_size * (2^num_qubits)
    
    # Create new state vector
    T = eltype(state)
    new_state = similar(state, new_size)
    fill!(new_state, zero(T))
    
    # Copy the old state vector into the new one
    # Only the basis states where the new qubits are all |0⟩ get the original amplitudes
