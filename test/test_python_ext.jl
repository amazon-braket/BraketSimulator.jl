...
@testset "Python integration" begin
    ...
    @testset "Gates" begin
        @testset "1q static" for (jl_gate, py_gate, py_sim_gate) in zip(
        ...
        angle2   = π / 5.2
        angle3   = π / 0.6
        @testset "1q 1-parameter" for (jl_gate, py_gate, py_sim_gate) in zip(
        ...
        @testset "1q 1-parameter" for (jl_gate, py_sim_gate) in zip(
            [
                GPi(angle),
                GPi2(angle),
        ...
        @testset "3-parameter" for (jl_gate, py_sim_gate, targets) in zip(
            [
        ...
        @testset "2q static" for (jl_gate, py_gate, py_sim_gate) in zip(
        ...
        @testset "2q 1-parameter" for (jl_gate, py_gate, py_sim_gate) in zip(
            [
                XX(angle),
                XY(angle),
                YY(angle),
        ...
        @testset "3q static" for (jl_gate, py_gate, py_sim_gate) in zip(
            [CCNot(), CSwap()],
    ...
    @testset "Noises" begin
        @testset "1q" for (jl_noise, py_noise, py_sim_noise) in zip(
                [
                    BitFlip(prob),
                    PhaseFlip(prob),
                    Depolarizing(prob),
        ...
        @testset "2q" for (jl_noise, py_noise, py_sim_noise) in zip(
